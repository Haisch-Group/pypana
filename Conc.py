...
def select_data(data, scan_nrs, used_C="Cn"):
    ...
    sel_results = pd.concat([sel_results, data["results"].iloc[py_nrs]], ignore_index=True)  # dropping NaN columns would need .dropna("all")

    sel_data = {used_C: sel_C, "el_time": sel_el_time, "filename": data["filename"], "used_device": data["used_device"],
                "add_info": sel_add_info, "results": sel_results}
    return sel_data
...
def plot_calc_conc_n(data, scan_nrs, used_C="calc_Cn", colors=Def.tum_cm, a=1, save_plot="off"):
    ...
    if len(py_nrs) == 1:
        k = py_nrs[0]
        ax.scatter(scan_nrs[0], calc_C[k], edgecolor="black", color=colors[0], alpha=a)
        # print(f"scan {k} conc. = " + "{:e}".format(float(calc_C[k])) + " P/cm" + u"\u00B3")
    else:
        for k in range(len(py_nrs)):
            ax.scatter(scan_nrs[k], calc_C[py_nrs[k]], edgecolor="black", color=colors[0], alpha=a)
            # print(f"scan {k} conc. = " + "{:e}".format(float(calc_C[k])) + " P/cm" + u"\u00B3")
            ct += 1
